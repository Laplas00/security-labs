...
from flask import request, render_template, redirect, url_for, session, flash, abort, current_app
...
@app.before_request
def smuggling_guard():
    shared_flag = current_app.config['ssrf_passed']
    print('shared flag===', shared_flag)
    if shared_flag['ssrf_passed'] == True:
        flash("✅ Blacklist bypassed via ipv6!")
...
def update_settings(user_id):
    ...
    if new_avatar_url and is_avatar_url_ok(new_avatar_url, vulnerability):
        ...
        requests.post('http://0.0.0.0:9000/write_log', data={'message':f'new avatar for {new_username}',
