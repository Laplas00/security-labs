    ...
    if request.method == 'POST':
        ...
        if f and f.filename:
            ...
            try:
                ...
                raw_fc  = draft.get('for_comments', True)

                if not title or not content:
                    raise ValueError("Empty title/content")

                # admins can never have comments
                if session.get('role') == 'admin':
                    for_comments = False
                else:
                    # type-confusion: any non-empty string or non-zero number is truthy
                    for_comments = bool(raw_fc)

            except Exception as e:
                flash(f'Error loading draft: {e}')
                return redirect(url_for('post_creation'))

            # сохраняем как новый пост, вместе с флагом for_comments
            db.execute(
                'INSERT INTO posts (title, content, author, for_comments) VALUES (?, ?, ?, ?)',
                (title, content, session['username'], for_comments)
            )
            db.commit()
            ...
            db.execute(
                'INSERT INTO posts (title, content, author, for_comments) VALUES (?, ?, ?, ?)',
                (title, content, author, show_comments)
            )
            db.commit()
            flash('Post added!')
            return redirect(url_for('post_creation'))
